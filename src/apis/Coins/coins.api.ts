...
export class CoinsApi extends Http {
  ...
  async fetchCoinInfoById(params: Api.Coins.FetchCoinInfoParams) {
    try {
      const res: Api.Response<Api.Coins.CoinInfo> = await this.get(
        '/v1/coin/show_coin_by_coinid',
        { ...params }
      );
      if (res.code === 1) {
        return res.data;
  ...
  // 获取代币价格
  async getCoinPrice(token_address: string) {
    const res = await this.get('/v1/coin/show_coin_by_address', {
      token_address
    });
    return res;
  }
