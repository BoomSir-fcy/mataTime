...
import {
  LETTER_ENGLISH_REGEXP,
  CONTENT_KEYWORD_REGEXP,
  CHINESE_WORD_REGEXP,
  SQUARE_REGEXP,
  HTTP_REGEXP,
  SYMBOL_REGEXP,
...
export const isEnglishContent = content => {
  ...
  const filterEnglishWordContent = sameContent.replace(
    LETTER_ENGLISH_REGEXP,
    '',
  );
...
export const getLanguageContent = content => {
  try {
    ...
    const filterEnglishWordContent = contentValue.replace(
      LETTER_ENGLISH_REGEXP,
      '',
    );
    const filterChineseWordContent = contentValue.replace(
      CHINESE_WORD_REGEXP,
      '',
    );
