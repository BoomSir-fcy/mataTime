...
const HoverUpdater = ({ locale, payload }) => {
  useEffect(() => {}, [locale, payload.value, payload.time]);
  return null;
};
...
const CustomTooltip = ({ locale, payload }) => {
  ...
  if (payload && payload.length) {
    return (
      <TooltipBox>
        <div>
          {t('AccountTime')}:{payload[0].payload.time}
        </div>
        <div>
          {t('Time Rewards')}:{payload[0].payload.value}
        </div>
      </TooltipBox>
    );
  }
    ...
    <ResponsiveContainer width='100%' height='100%'>
      ...
      >
        ...
        <Tooltip
          cursor={false}
          content={props => (
            <CustomTooltip locale={locale} payload={props.payload} />
          )}
        />
        <Area
          dataKey='value'
