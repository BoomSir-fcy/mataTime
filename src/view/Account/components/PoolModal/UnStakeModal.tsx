...
const UnstakeBtn: React.FC<{
  ...
  return (
    ...
    >
      {pendingTx ? <Dots>{t('Unstaking')}</Dots> : t('Unstaked')}
    </Button>
  );
};
...
const UnStakeModal: React.FC<UnStakeModalProps> = ({
  ...
  const stakesList = useMemo(() => {
    return stakes.map(item => {
      ...
      const token0UnclaimedRewards = new BigNumber(
        token0UnclaimedRewardsAsString,
      );
      const token1UnclaimedRewards = new BigNumber(
        token1UnclaimedRewardsAsString,
      );
      const stakeAmount = new BigNumber(stakeAmountAsString);

      const dispalyToken0Rewards = formatDisplayBalanceWithSymbol(
        token0UnclaimedRewards,
        rewardToken1Decimals,
      );
      const dispalyToken1Rewards = formatDisplayBalanceWithSymbol(
        token1UnclaimedRewards,
        rewardToken0Decimals,
      );
      const dispalyStakeAmount = formatDisplayBalanceWithSymbol(
        stakeAmount,
        depositDecimals,
      );
  ...
  }, [stakes, depositDecimals, rewardToken0Decimals, rewardToken1Decimals]);
            ...
            {Number(item.unlockTime) * 1000 <= nowTime ? (
              ...
              <Box>
                ...
                <Text textAlign='right'>
                  {dayjs(Number(item.unlockTime) * 1000).format(
                    'YYYY-MM-DD HH:mm',
                  )}
                </Text>
              </Box>
