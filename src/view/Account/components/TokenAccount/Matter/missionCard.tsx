...
import { Button, Heading, Text, Flex, Svg } from 'uikit'
          ...
          < MisCard >
            ...
            <Info>
              ...
              <Flex justifyContent='space-between' alignItems='center'>
                <Text color='textPrimary'><span style={{ fontWeight: 'bold', fontSize: '16px' }}>+{info.points}</span> Points</Text>
                <Button disabled={pengdingType || info.taskStatus === 3 || ((info.taskName === 'CompletedAllDailyTasksAndAdvancedTasks' || info.taskName === 'ContinuousSignIn') && info.taskStatus === 1)} onClick={async () => {
                  ...
                  {info.taskStatus === 3 ? t('Claimed') : info.taskStatus === 2 ? t('Claim2') : (info.taskName === 'CompletedAllDailyTasksAndAdvancedTasks' || info.taskName === 'ContinuousSignIn') ? t('To be completed') : t('To finish')}
                </Button>
              </Flex>
            </Info>
            {info.continuous ?
              <div>
                <Flex mb='8px' justifyContent='space-between' alignItems='center'>
                  <Text small>{t('Progress')}</Text>
                  <Text small>{`${info.continuous?.now}/${info.continuous?.max}`}{getName(info.taskName)}</Text>
                </Flex>
                {/* <Progress scale='sm' variant='round' primaryStep={(info.continuous.now / info.continuous?.max) * 100} /> */}
              </div>
              :
              <Flex justifyContent='space-between' alignItems='center'>
                <Text color='textTips' fontSize='14px'>{t('Quest ends in')}</Text>
                <Flex alignItems='center'>
                  <CountdownTime endTime={info.endTime} />
